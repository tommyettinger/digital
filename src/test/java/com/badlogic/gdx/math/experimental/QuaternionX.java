 ...
 * @author vesuvio
 * @author xoppa */
public class QuaternionX implements Serializable {
	private static final long serialVersionUID = -7661875440774897168L;
	private static QuaternionX tmp1 = new QuaternionX(0, 0, 0, 0);
	private static QuaternionX tmp2 = new QuaternionX(0, 0, 0, 0);
	...
	public QuaternionX(QuaternionX quaternion) {
		this.set(quaternion);
	}
	...
	public QuaternionX(Vector3 axis, float angle) {
		this.set(axis, angle);
	}
	...
	public QuaternionX set (float x, float y, float z, float w) {
		this.x = x;
		this.y = y;
		this.z = z;
		this.w = w;
	 ...
	 * @return This quaternion for chaining. */
	public QuaternionX set (QuaternionX quaternion) {
	 ...
	 * @param angle The angle in degrees
	 * @return This quaternion for chaining. */
	public QuaternionX set (Vector3 axis, float angle) {
		return setFromAxis(axis.x, axis.y, axis.z, angle);
	}

	/** @return a copy of this quaternion */
	public QuaternionX cpy () {
		return new QuaternionX(this);
	}
	...
	public QuaternionX setEulerAngles (float yaw, float pitch, float roll) {
	 ...
	 * @return this quaternion */
	public QuaternionX setEulerAnglesRad (float yaw, float pitch, float roll) {
	...
	public float getRollRad () {
		return MathUtils.atan2((w * z + y * x), 0.5f - (x * x + z * z));
//		final int pole = getGimbalPole();
//		return pole == 0 ? MathUtils.atan2((w * z + y * x), 0.5f - (x * x + z * z))
//			: (float)pole * 2f * MathUtils.atan2(y, w);
	...
	public float getPitchRad () {
		return (float)Math.asin(MathUtils.clamp(2f * (w * x - z * y), -1f, 1f));
//		final int pole = getGimbalPole();
//		return pole == 0 ? (float)Math.asin(MathUtils.clamp(2f * (w * x - z * y), -1f, 1f)) : (float)pole * MathUtils.PI * 0.5f;
	...
	public float getYawRad () {
		return MathUtils.atan2((y * w + x * z), 0.5f - (y * y + x * x));
		//return getGimbalPole() == 0 ? MathUtils.atan2((y * w + x * z), 0.5f - (y * y + x * x)) : 0f;
	}
	 ...
	 * 
	 * @param other QuaternionX to multiply with
	 * @return This quaternion for chaining */
	public QuaternionX mul (final QuaternionX other) {
	 ...
	 * @return This quaternion for chaining */
	public QuaternionX mul (final float x, final float y, final float z, final float w) {
	 ...
	 * @param other QuaternionX to multiply with
	 * @return This quaternion for chaining */
	public QuaternionX mulLeft (QuaternionX other) {
	 ...
	 * @param w the w component of the other quaternion to multiply with
	 * @return This quaternion for chaining */
	public QuaternionX mulLeft (final float x, final float y, final float z, final float w) {
	...
	public QuaternionX add (QuaternionX quaternion) {
		this.x += quaternion.x;
		this.y += quaternion.y;
		this.z += quaternion.z;
		this.w += quaternion.w;
	...
	/** Add the x,y,z,w components of the passed in quaternion to the ones of this quaternion */
	public QuaternionX add (float qx, float qy, float qz, float qw) {
		this.x += qx;
		this.y += qy;
		this.z += qz;
		this.w += qw;
...
// method
	...
	/** Sets the quaternion to an identity QuaternionX
	 * @return this quaternion for chaining */
	public QuaternionX idt () {
		return this.set(0, 0, 0, 1);
	}

	/** @return If this quaternion is an identity QuaternionX */
	 ...
	 * @return This quaternion for chaining. */
	public QuaternionX setFromAxis (final Vector3 axis, final float degrees) {
	 ...
	 * @return This quaternion for chaining. */
	public QuaternionX setFromAxisRad (final Vector3 axis, final float radians) {
	 ...
	 * @param degrees The angle in degrees
	 * @return This quaternion for chaining. */
	public QuaternionX setFromAxis (final float x, final float y, final float z, final float degrees) {
	 ...
	 * @param radians The angle in radians
	 * @return This quaternion for chaining. */
	public QuaternionX setFromAxisRad (final float x, final float y, final float z, final float radians) {
	...
	/** Sets the QuaternionX from the given matrix, optionally removing any scaling. */
	public QuaternionX setFromMatrix (boolean normalizeAxes, Matrix4 matrix) {
	...
	/** Sets the QuaternionX from the given rotation matrix, which must not contain scaling. */
	public QuaternionX setFromMatrix (Matrix4 matrix) {
		return setFromMatrix(false, matrix);
	}

	/** Sets the QuaternionX from the given matrix, optionally removing any scaling. */
	public QuaternionX setFromMatrix (boolean normalizeAxes, Matrix3 matrix) {
	...
	/** Sets the QuaternionX from the given rotation matrix, which must not contain scaling. */
	public QuaternionX setFromMatrix (Matrix3 matrix) {
		return setFromMatrix(false, matrix);
	}

	/**
	 * <p>
	 * Sets the QuaternionX from the given x-, y- and z-axis which have to be orthonormal.
	 * </p>
	 * 
	 * <p>
	...
	public QuaternionX setFromAxes (float xx, float xy, float xz, float yx, float yy, float yz, float zx, float zy, float zz) {
		return setFromAxes(false, xx, xy, xz, yx, yy, yz, zx, zy, zz);
	}

	/**
	 * <p>
	 * Sets the QuaternionX from the given x-, y- and z-axis.
	 * </p>
	 * 
	 * <p>
	 * Taken from Bones framework for JPCT, see http://www.aptalkarga.com/bones/ which in turn took it from Graphics Gem code at
	 ...
	 * @param zy z-axis y-coordinate
	 * @param zz z-axis z-coordinate */
	public QuaternionX setFromAxes (boolean normalizeAxes, float xx, float xy, float xz, float yx, float yy, float yz, float zx,
									float zy, float zz) {
		if (normalizeAxes) {
	 ...
	 * @return this quaternion for chaining */
	public QuaternionX slerp (QuaternionX end, float alpha) {
	...
	/** Spherical linearly interpolates multiple quaternions and stores the result in this QuaternionX. Will not destroy the data
	 * previously inside the elements of q. result = (q_1^w_1)*(q_2^w_2)* ... *(q_n^w_n) where w_i=1/n.
	 * @param q List of quaternions
	 * @return This quaternion for chaining */
	public QuaternionX slerp (QuaternionX[] q) {
	...
	/** Spherical linearly interpolates multiple quaternions by the given weights and stores the result in this QuaternionX. Will
	 ...
	 * @return This quaternion for chaining */
	public QuaternionX slerp (QuaternionX[] q, float[] w) {
	 ...
	 * @return This quaternion for chaining */
	public QuaternionX exp (float alpha) {
...
// value
	...
	public boolean equals (Object obj) {
		...
		if (!(obj instanceof QuaternionX)) {
			return false;
		}
		QuaternionX other = (QuaternionX)obj;
	...
	public float dot (final QuaternionX other) {
	 ...
	 * @return this quaternion for chaining. */
	public QuaternionX mul (float scalar) {
		this.x *= scalar;
		this.y *= scalar;
		this.z *= scalar;
		this.w *= scalar;
	...
	public void getSwingTwist (final float axisX, final float axisY, final float axisZ, final QuaternionX swing,
		final QuaternionX twist) {
	 ...
	 * @param twist will receive the twist rotation: the rotation around the specified axis
	 * @see <a href="http://www.euclideanspace.com/maths/geometry/rotations/for/decomposition">calculation</a> */
	public void getSwingTwist (final Vector3 axis, final QuaternionX swing, final QuaternionX twist) {
